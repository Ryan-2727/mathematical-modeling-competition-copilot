...
- 单一机器可读的 CUMCM 2026 规则配置与 T-30/T-7/T-1 新鲜度检查、官方
  ...
  冻结文件与客户端 MD5 实值比对、官方双指标相似度证据，以及
  P90/趋势/连续两次完整通过的训练就绪度评分
- 正式比赛采用精确联网边界：可检索官方、学术和静态参考资料，但禁止在
  交流平台浏览当前赛题相关内容；分类不确定时暂停并询问用户，同时不设置
  搜索词字面黑名单
...
- `prepare_private_regression.py` 盘点用户自有历史题库，并只将显式白名单
  文件复制到与题库、公开仓库均不重叠的私有回归目录。默认盘点仅发现案例；选定
  的校准题才显式检查。它拒绝路径逃逸和已生成
  的解答工件；私有清单、题面、数据、输出和评分均不得提交到 Git。
- `probe_runtime_capabilities.py` 在选模前记录可用求解器/运行时的确切版本；
  缺少能力必须显式标记，不能静默替换算法或自动安装依赖。
- `verify_data_cache.py` 将大数据聚合缓存与原始数据/缓存哈希、聚合规则和
...
- `verify_answer_density.py` 要求摘要和结论对每个子问题给出直接结果或建议、验证与
