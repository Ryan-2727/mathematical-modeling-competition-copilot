...
def main() -> int:
    ...
    write_if_missing(root / "reports/model_decision_log.csv", "subproblem,model_level,parent_model,baseline,candidate,added_mechanism,mechanism_fit,assumptions,new_parameters,expected_diagnostic_signature,failure_test,validation_cost,identifiability_status,selected,selection_evidence,status\n")
    ...
    write_if_missing(root / "reports/parameter_registry.csv", "subproblem,model_id,parameter,symbol,role,unit,scope,source,bounds,identifiability_status,claim_boundary,status,claim_sensitive,source_class,source_locator,source_sha256,citation_key,calibration_command,sensitivity_evidence,paper_location\n")
    write_if_missing(root / "reports/independent_routes.csv", "subproblem,route_id,route_role,principle,data_representation,failure_mode,result_file,result_value,tolerance,comparison_status,limitation,status\n")
    ...
    write_if_missing(root / "reports/paper_reasoning_map.csv", "subproblem,paper_location,modeling_path,modeling_path_evidence,modeling_path_evidence_sha256,model_choice_required,model_choice_location,parameter_location,failed_route_required,failed_route_location,boundary_required,boundary_location,human_reviewer,status\n")
