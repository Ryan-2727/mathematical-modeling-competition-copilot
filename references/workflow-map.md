...
- `local-originality-preflight.md`: complete-LaTeX local corpus scan, exact and
  near-match localization, hash-bound human review, and official-check boundary.
...
| Writing | `paper/main.tex`, `paper/main.pdf`, generated result fragments, `reports/paper_reasoning_map.csv`, numeric and prose exception ledgers, `reports/originality_preflight.md`, notation/dimension registry, complete portable LaTeX source tree | Every abstract/conclusion number is a verified macro or narrow structural exemption; evidence-triggered model choice, parameter provenance, real failed runs, and boundaries have natural paper locations and a named human reviewer; Chinese prose and local-corpus HIGH/MEDIUM findings are located for human review; symbols and units reconcile; Overleaf-style root and VS Code `build/` outputs pass |
